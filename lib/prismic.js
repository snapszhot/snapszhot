...
export async function getPrismicPrefills(previewData) {
    const { results } = await Client(previewData?.ref).getByType('csv', {
        pageSize: 1,
        page: 1,
    })

    return await getPrefills(results[0].data.csv_file.url)
}
