...
export function getGames(hasMounted) {
    if (hasMounted) {
...
export function getPlayedGames(includeResult = false, hasMounted) {
    if (hasMounted) {
...
export function updateStats({ hasMounted, playerWon, todaysGuesses }) {
    if (hasMounted) {
...
export function updateSettings({ enabledHints, hasMounted }) {
    if (hasMounted) {
...
export function initializeStorage(day, preview, hasMounted) {
    if (hasMounted) {
        const games = getGames(hasMounted)
        ...
        if (games && selectedGame && !preview) {
            ...
            // We have to check guess.movie and guess.originalTitle to honor
            // old guesses with the guess.movie format.
            const guesses =
                initGuesses === ''
                    ? initGuesses
                    : JSON.parse(initGuesses).map(guess => {
                          const originalTitle = guess?.movie
                              ? guess.movie
                              : guess.originalTitle
